...
- **2026-08-31** — **CJK in table PNGs + display-width column sizing** (`render_table.ts`, `format.ts`, new `text_width.ts`). Repro: a Chinese reimbursement table sent to `#eccv-reimburse` came back with every Han character blank; Latin/`€`/digits were fine. **Two independent root causes that had to be fixed together** — font alone leaves the columns cramped, width alone leaves them blank:

  1. **No CJK face was vendored.** `SATORI_FONTS` held only DejaVu Sans/Bold/Mono, none of which carry Han/Kana/Hangul glyphs, and `loadAdditionalAsset` returns `''` for every `code` except `'emoji'` — so satori's missing-glyph path had nothing to fall back to and emitted `.notdef` boxes. Same failure class as the 2026-05-12 emoji bug, just never extended past emoji. Fix: vendor `fonts/NotoSansSC-Regular.otf` (8.3 MB, `notofonts/noto-cjk` → `Sans/SubsetOTF/SC/NotoSansSC-Regular.otf`) and put it in a CSS fallback stack, `FONT_STACK = 'DejaVu Sans, Noto Sans SC'`, on the root node (plus `'DejaVu Sans Mono, Noto Sans SC'` on `inlineMd`'s code spans, so `` `code` `` cells with CJK work too).

     **Two non-obvious registration rules, both load-bearing:**
     - It must be registered under its **own family name**. Adding it as a second `'DejaVu Sans'` entry does **not** chain — satori resolves a family to the first matching registration and keeps emitting tofu. The fallback has to live in the CSS `fontFamily` stack, not in the font list.
     - It must be registered at **weight 700 as well as 400**. Header cells are `fontWeight: 600` and `**bold**` spans are `700`; with only a 400 entry satori resolves those runs to DejaVu Bold and **bold CJK regresses to tofu even though regular CJK works**. We reuse the Regular bytes for the 700 entry rather than vendoring `NotoSansSC-Bold.otf` (+8.1 MB) — see Known limitations for the cosmetic cost.

  2. **Columns were sized in UTF-16 code units, not display cells.** `visibleLen` (`render_table.ts`) returned `.length`, and widths are `chars × CHAR_PX (7.5)`; a Han ideograph is 1 code unit but ~14 px at `FONT_PX 14`, so every CJK column got about half the space it needed and its cells wrapped. `normalizeTableWidths` (`format.ts`) padded with `.padEnd` on the same count, so **the `table-N.md` fallback attachment misaligned by one cell per ideograph too** — both outputs wrong, in different ways, from one miscount. Fix: new **`text_width.ts`** exporting `displayWidth` / `padEndWidth` / `isFullWidthCodePoint`, consumed by `visibleLen`, `countWrappedLines`, and `normalizeTableWidths`.

     **`text_width.ts` design notes:** East_Asian_Width is *not* available to JS regex (`\p{…}` covers only General_Category / Script / binary properties), so the Wide+Fullwidth ranges are spelled out. Two deliberate choices: (a) **every astral codepoint (> U+FFFF) counts 2** — correct for emoji and CJK Ext-B, and behaviour-preserving, since `.length` already charged a surrogate pair 2, so no existing Latin column shifted; (b) the classic `is-fullwidth-code-point` range table is **Unicode 5.2-era and misses the emoji-presentation symbols that Unicode 9.0 promoted to Wide**, so `isWideEmoji` adds them (U+2705 `✅`, U+274C `❌`, U+2B50, …). Without (b) a `✅`-bearing row sat one cell short in the aligned `.md`. U+2764 `❤` and U+26A0 `⚠` are EAW=Neutral and deliberately stay at 1.

  **Verification:** `bun test` 116/116 (95 pre-existing + 21 new across `tests/text_width.test.ts`, `tests/format.test.ts`, `tests/render_table.test.ts`), plus an end-to-end render through `buildReplyMessages` of a mixed CJK/Latin table with a bold CJK row, CJK inside an inline-code span, kana, and a `✅`. The load-bearing automated check is a **tofu detector**: render two tables differing only in *which* Han character they contain (`一一` vs `龘龘`) — with no CJK face both collapse to the same `.notdef` box and the PNGs are byte-identical, so `expect(a.equals(b)).toBe(false)` fails loudly if the font is ever dropped from the checkout. A `pure-ASCII tables are byte-identical` test pins the no-regression claim for existing Latin tables.

  **Cost:** the vendored font roughly doubles plugin checkout size (8.3 MB). If that ever matters, subset to the CJK ranges actually in use rather than reverting the stack.
...
- **Bold CJK is not visually bolder.** `SATORI_FONTS` registers the *same* `NotoSansSC-Regular.otf` bytes at weight 400 and 700 (see the 2026-08-31 patch under Local patches for why the 700 entry is mandatory). That fixes bold-CJK tofu but gives no real bold outlines — `**住宿费用**` renders at regular weight while the Latin in the same row is genuinely bold. Closing it means vendoring `NotoSansSC-Bold.otf` (+8.1 MB, doubling the CJK font cost for a purely cosmetic gain). Deferred.

- **`CHAR_PX = 7.5` under-budgets digit-heavy Latin cells.** Pre-existing, unrelated to CJK, and confirmed 2026-08-31 with a pure-ASCII control table: `€1,250.00` (9 visible chars → 67.5 px budget) wraps to two lines because DejaVu's digits are ~8.9 px wide at `FONT_PX 14`. Any fix means raising `CHAR_PX` (widens *every* table) or measuring real glyph advances from the font — see the 2026-05-11 A1 note for why the cheap constant-bump was rejected before.
